...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(number_of_points+1):
        x=circle.center.x+circle.radius*math.cos(angle*k)
        y = circle.center.y + circle.radius * math.sin(angle*k)
        point=rg.Point(x,y)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  15 slices, thin (thickness=3) yellow lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color='orange'
    circle.outline_thickness = 3
    pizza(window, circle, 15, 'yellow', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    Type hints:
      ...
      :type thickness:        int
    """
    point_on_circle=generate_points_on_circle(circle,number_of_slices)
    circle.attach_to(window)
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 7, 'yellow', 15)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    """
    circle.attach_to(window)
    point_on_circle=generate_points_on_circle(circle,number_of_segments)
    for k in range(len(point_on_circle)):
        point=point_on_circle[k]
        if k+1>len(point_on_circle)-1:
            point2=point_on_circle[0]
        else:
            point2=point_on_circle[k+1]
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 20, 5, 'yellow', 5)
    window.close_on_mouse_click()
    # -------------------------------------------------------------------------
    # Test 4:  (YOU write THIS test).
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    point_on_circle = generate_points_on_circle(circle, number_of_lines)
    for k in range(number_of_lines):
        start=point_on_circle[k]
        end=point_on_circle[(k-n)%len(point_on_circle)]
